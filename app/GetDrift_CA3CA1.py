...
class twolayer_FF:
    def __init__(self, n_inp, nCA1,nCA3,eca1,eca3,tau=10.0, dt=1.0, act=torch.relu, lr=8e-6, decay_r=1e-2, threshold=0, I0=5, I1=0.05, I2=0.01):
        self.nCA1 = nCA1
        ...
        self.W_inp_CA1 = torch.abs(torch.ones(n_inp, nCA1)) / n_inp**0.5
        self.W_inp_CA3 = torch.abs(torch.ones(n_inp, nCA3)) / n_inp**0.5
    ...
    def step(self, input_FR):
        ...
        I_inhib_ca1 = self.I0 + self.I1 * torch.sum(self.r_CA1) + self.I2 + torch.sum(self.r_CA1**2)

        # blanket inhibition to the RNN
        I_inhib_ca3 = self.I0 + self.I1 * torch.sum(self.r_CA3) + self.I2 + torch.sum(self.r_CA3**2)


        # rate change as the nonlinear ODE
        dr_ca1_dt = (-self.r_CA1 +  self.act(tot_inp_CA1  - I_inhib_ca1 + self.excitability)) / self.tau
        dr_ca3_dt = (-self.r_CA1 +  self.act(total_inp_CA3  - I_inhib_ca3 + self.excitability_CA3 )) / self.tau
        ...
        hebbian_dw_ca1 = self.lr * self.excitability * torch.outer(input_FR, self.r_CA1*post_mask_ca1) * self.dt
        # hebbian plasticity in inp -> CA3 weights
        hebbian_dw_ca3 = self.lr * self.excitability_CA3* torch.outer(input_FR, self.r_CA3*post_mask_ca3) * self.dt
        # hebbian plasticity in CA3 -> CA1 weights
        hebbian_dw_ca3_ca1 = self.lr * self.excitability * torch.outer(self.r_CA3, self.r_CA1*post_mask_ca1) * self.dt
        # hebbian plasticity in CA3 -> CA3 weights
        hebbian_dw_ca3_ca3 = self.lr * self.excitability_CA3 * torch.outer(self.r_CA3, self.r_CA3*post_mask_ca3) * self.dt
...
nonFC_input = torch.abs(torch.normal(mean=0.0, std=3.0, size=(ni,)))
# nonFC_input = 
# nonFC_input[10:] += 1
CA1_FR_history = []
CA3_FR_history = []
