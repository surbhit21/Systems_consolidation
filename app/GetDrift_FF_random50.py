...
def main():
    ...
    n_offline_reactivated = int(round(n * offline_reactivation_fraction))
    ...
    for i in trange(NUM_SIM):
        ...
        for day in range(N_off_days):
            ...
            # 50% of principal-population neurons without replacement.
            if day == 0 or day == N_off_days - 1:
                ...
                reactivated = np.random.choice(
                    n, size=n_offline_reactivated, replace=False
                )
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_folder, "pca_cosine_similarity_to_day0.png"), dpi=300)
    plt.close()
    ...
    plot_corr_matrix(last_activity, fname=os.path.join(plot_folder, "corr_matrix.png"))
    ...
    mean_corr, std_corr, per_sim_corr, idx = plot_mean_std_corr_over_time(
        ...
        fname=os.path.join(plot_folder, "encoding_vs_others_mean_std.png"),
        cmap="Reds",
        bar_plot=True
    )
    ...
    last_activity_all, std_corr, per_sim_corr, idx = plot_mean_std_corr_over_time(
        ...
        include_ref_bar=False,
        title="Encoding vs. others (mean ± SD across sims)",
        fname=os.path.join(plot_folder, "recall_vs_others_mean_std.png"),
        cmap="Reds",
        bar_plot=True
