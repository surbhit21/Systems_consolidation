...
class twolayer_FF:
    ...
    def step(self, input_FR, op_signal = 0,op_inp = 0):
        ...
        I_inhib_ctx = self.I0 + self.I1 * np.sum(self.rates_ctx) + self.I2 * np.sum(self.rates_ctx**2)
        ...
        hebbian_dw = self.on * self.lr * np.outer(self.rates*post_mask, self.rates*post_mask) * self.dt
        ...
        hebbian_dw_ctx = self.on_ctx * self.lr_ctx * np.outer(self.rates_ctx*post_mask, self.rates_ctx*post_mask) * self.dt
    ...
    def _normalize_input_outgoing(self, target_sum=None, eps=1e-12):
        ...
        col_sums = self.rec_w.sum(axis=1, keepdims=True)  # shape (1, n_inp)
...
for x1,x2 in zip(E_fl,E_fl_ctx):
    ...
    for i in trange(NUM_SIM):
        ...
        day = 0
        nn.excitability[off_set+(day)*20:off_set+(day)*20+20] += x1
