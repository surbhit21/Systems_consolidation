...
class twolayer_FF:
    def __init__(self, n_inp, n_neurons,n_cont,baseline_e,tau=20.0, dt=1.0, act=None, lr=1/800, decay_r=1/1000, I0=1, I1=0.05, I2=0.001):
        ...
        self.act_threshold = 0#np.abs(np.random.normal(0,0.7,size=(n_neurons,)))
        self.n_cont = n_cont
        self.cont_exc = np.abs(np.random.normal(0,1,size=(self.n_cont,)))
        self.act_threshold_cont = 0#np.abs(np.random.normal(0,0.7,size=(self.n_cont,)))
...
def main():
    ...
    # base_E[:off_set] += 2
    FC_inp = 25
    input = 15*np.ones(n_inp)
    cont_inp = 15*np.ones(n_cont)
    ...
    os.makedirs("./data/Reimagined_3", exist_ok=True)
    os.makedirs("./plots/Reimagined_3", exist_ok=True)
    np.save("./data/Reimagined_3/FR_history.npy",FR_history_all)
    np.save("./data/Reimagined_3/EX_history.npy",EX_history_all)
    np.save("./data/Reimagined_3/rec_weights.npy", rec_weights_all)
    ...
    np.save("./data/Reimagined_3/pca_scores_reactivation.npy", pca_scores)
    np.save("./data/Reimagined_3/pca_components_reactivation.npy", pca_components)
    np.save("./data/Reimagined_3/pca_mean_activity_reactivation.npy", pca_mean_activity)
    np.save("./data/Reimagined_3/pca_explained_variance_reactivation.npy", pca_explained_variance)
    np.save("./data/Reimagined_3/pca_cosine_similarity_to_day0.npy", pca_cosine_to_day0)
    ...
    plt.tight_layout()
    plt.savefig("./plots/Reimagined_3/pca_cosine_similarity_to_day0", dpi=300)
    plt.close()
    ...
    plot_corr_matrix(last_activity, fname="./plots/Reimagined_3/corr_matrix")
    ...
    np.save(
        "./data/Reimagined_3/first_session_engram_firing_rates.npy",
        first_engram_rates,
    )
    np.save(
        "./data/Reimagined_3/first_session_engram_active_session_counts.npy",
    ...
    scatter_rates, scatter_counts, persistence_r = (
        plot_first_activity_vs_active_sessions(
            ...
            fname="./plots/Reimagined_3/first_session_rate_vs_engram_sessions",
        )
    )
    ...
    fig.tight_layout()
    save_plot("./plots/Reimagined_3/first_session_rate_by_engram_sessions")
    plt.close(fig)
    ...
    with open(
        "./data/Reimagined_3/engram_persistence_summary.json", "w"
    ) as handle:
    ...
    mean_corr, std_corr, per_sim_corr, idx = plot_mean_std_corr_over_time(
        ...
        fname="./plots/Reimagined_3/encoding_vs_others_mean_std",
        cmap="Reds",
        bar_plot=True
    )
    ...
    last_activity_all, std_corr, per_sim_corr, idx = plot_mean_std_corr_over_time(
        ...
        include_ref_bar=False,
        title="Encoding vs. others (mean ± SD across sims)",
        fname="./plots/Reimagined_3/recall_vs_others_mean_std",
        cmap="Reds",
        bar_plot=True
