...
class twolayer_FF:
    def __init__(self, n_inp, n_neurons, n_cont, baseline_e,
                 stimulus_neuron_indices=None, tau=20.0, dt=1.0, act=None,
                 lr=1/800, decay_r=1/1000, I0=1, I1=0.05, I2=0.001):
        if act is None:
            act = softplus
        ...
        if stimulus_neuron_indices is None:
            stimulus_neuron_indices = np.array([], dtype=int)
        self.stimulus_neuron_indices = np.asarray(
            stimulus_neuron_indices, dtype=int
        )
        self.memory_neuron_mask = np.ones(n_neurons, dtype=float)
        self.memory_neuron_mask[self.stimulus_neuron_indices] = 0.0
        ...
        # self.
    def step(self, input_FR, cont_INP, offline=False):
        """
        ...
        if offline:
            # Embedded stimulus neurons represent the sensory cue and must be
            # silent during offline reactivation, even if population activity
            # would otherwise recruit them indirectly.
            self.rates[self.stimulus_neuron_indices] = 0.0
...
def main():
    ...
    IR = 100
    Nrep = 10
    n_stimulus_neurons = 8
    stimulus_neuron_indices = np.arange(n_stimulus_neurons, dtype=int)
    stimulus_input_amplitude = 21.0
    other_input_amplitude = 17.0
    ...
    offline_reactivation_fraction = 1.0
    n_offline_reactivated = int(
        round(offline_candidate_indices.size * offline_reactivation_fraction)
    )
    data_folder = "./data/Reimagined_3_embedded_stimulus_all_memory"
    plot_folder = "./plots/Reimagined_3_embedded_stimulus_all_memory"
    excitability_cohorts = np.array_split(
        offline_candidate_indices, N_off_days
    )
    ...
    for i in trange(NUM_SIM):
        ...
        nn = twolayer_FF(n_inp=n_inp,
                        ...
                        n_cont=n_cont, 
                        baseline_e=base_E,
                        stimulus_neuron_indices=stimulus_neuron_indices,
                        tau=20.0, dt=1, 
                        act=relu, 
        ...
        for day in range(N_off_days):
            ...
            nn.excitability[excitability_cohorts[day]] += E_fl
            nn.excitability[stimulus_neuron_indices] = base_E[
                stimulus_neuron_indices
            ]
            # Encoding (day 0) and recall (final day) use the full cue. Each
            # intervening offline session reactivates every non-stimulus neuron
            # at the same amplitude used during encoding. Reserved stimulus
            # neurons are never externally driven during offline reactivation.
            if day == 0 or day == N_off_days - 1:
                reactivation_mask = np.ones(n, dtype=bool)
            else:
                reactivation_mask = np.zeros(n, dtype=bool)
                reactivation_mask[offline_candidate_indices] = True
            ...
            session_is_offline = 0 < day < N_off_days - 1
            if day == 0 or day == 6:
            ...
            for t in range(ID):
                next_FR = nn.step(
                    zero_input, zero_cont_inp, offline=True
                )
                FR_history.append(next_FR)
                EX_history.append(nn.excitability.copy())
                input_history.append(zero_input)
    ...
    sim_params = {
        ...
        "stimulus_recurrent_connections": False,
        "stimulus_excitability_drift": False,
        "stimulus_clamped_silent_offline": True,
