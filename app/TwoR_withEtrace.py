...
def main():
    ...
    acc_ltp_erasure_days = [1]
    off_days = [0, 1, 2, 3, 7, 10]

    # Build a descriptive simulation name from blockade and plasticity settings.
    if not dob_ACC == []:
        condition += "ACCBlock{}_withLateACCLR".format(dob_ACC)
    if not dob_HPC == []:
        condition += "HPCBlock{}_withLateACCLR".format(dob_HPC)
    if not ip_block_days == []:
        condition += "IPBlock{}_withLateACCLR".format(ip_block_days)
    if not hpc_ltp_erasure_days == []:
        condition += "HPCLTPErase{}_withLateACCLR".format(hpc_ltp_erasure_days)
    if not acc_ltp_erasure_days == []:
        condition += "ACCLTPErase{}_withLateACCLR".format(acc_ltp_erasure_days)
    if condition == "":
        condition += "CNT_withLateACCLR"
    if E_mod == 0.0:
        sim_name = "{}_fast_drift_wo_IP_lowI_withLateACCLR".format(condition)
    else:
        sim_name = "{}_fast_drift_with_limited{}_IP_lowI_withLateACCLR".format(condition, IP_plasticity_limit)
