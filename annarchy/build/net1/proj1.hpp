...
/////////////////////////////////////////////////////////////////////////////
// proj1: E_pop -> op_pop with target exc
/////////////////////////////////////////////////////////////////////////////
extern struct ProjStruct1 *proj1;
struct ProjStruct1 : LILMatrix<int, int> {
    ProjStruct1() : LILMatrix<int, int>( 1, 70) {
        // HACK: the object constructor is now called by nanobind, need to update reference in C++ library
        proj1 = this;

    #ifdef _TRACE_INIT
        std::cout << "  ProjStruct1 - this = " << this << " has been allocated." << std::endl;
    #endif
    }
    ...
    #ifdef _TRACE_INIT
        std::cout << "  ProjStruct1::init_projection(post_size = " << pop1->size << ", pre_size = " << pop0->size << ") - this = " << this << std::endl;
    #endif
        ...
        if(_transmission && _update && pop1->_active && ( (t - _update_offset)%_update_period == 0L) ){
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    double _w = (freeze[i] ? 0 : pop1->r[rk_post]*pop0->r[rk_pre]*lr/tau_w - w[i][j]/tau_decay);
    ...
    std::vector<bool> get_semiglobal_attribute_all_bool(std::string name) {
        ...
        // should not happen
        std::cerr << "ProjStruct1::get_semiglobal_attribute_all_bool: " << name << " not found" << std::endl;
    ...
    bool get_semiglobal_attribute_bool(std::string name, int rk_post) {
        ...
        // should not happen
        std::cerr << "ProjStruct1::get_semiglobal_attribute_bool: " << name << " not found" << std::endl;
        return 0.0;
    }
